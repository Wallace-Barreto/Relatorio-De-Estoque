...
void escrever_produtos(ofstream &arquivo, const Relatorio &rel)
{
    ...
    for (int i = 0; i < rel.total_produtos; i++)
    {
        ...
        int qtd = 1;
        rel.objetos[i].valor_total = calcular_valor_total(&rel.objetos[i], &qtd);
...
void gerador_relatorio()
{
    ...
    // Inicializa Relatorio corretamente
    ...
    preencher_campos_relatorio(*relatorio);
