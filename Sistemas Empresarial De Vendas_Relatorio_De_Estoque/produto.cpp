...
float calcular_valor_total(const mercadorias *produtos, int *total)
{
    ...
    float valor = produtos->qtd_atual * produtos->preco_custo;
    (*total)--;

    return valor + calcular_valor_total(produtos + 1, total);
}
...
void carregarDadosAnteriores(const char *nomeArquivo)
{
    ...
    if (!arquivo)
    {
        cout << "Arquivo de dados anteriores nao encontrado. Criando novo arquivo...\n";
        return;
    }
